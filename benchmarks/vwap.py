...
def bench(n, dtype):
    ...
    s = time.perf_counter()
    ta.volume.VolumeWeightedAveragePrice(data, data, data, data).volume_weighted_average_price()
...
plt.xlabel('n')
plt.ylabel('speedup')
plt.title('Volume Weighted Average Price (VWAP) Speedup')
plt.legend(loc='upper right')
plt.savefig('results/vwap.svg')