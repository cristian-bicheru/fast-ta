...
def bench(n, dtype):
    ...
    s = time.perf_counter()
    fast_ta.volume.ADI(data, data, data, data)
    times[0] = time.perf_counter()-s
    
    data = pandas.Series(data)
    s = time.perf_counter()
    ta.volume.AccDistIndexIndicator(data, data, data, data).acc_dist_index()
...
if __name__ == '__main__':
    ...
    plt.xlabel('n')
    plt.ylabel('speedup')
    plt.title('Accumulation/Distribution Index (ADI) Speedup')
    plt.legend(loc='upper right')
    plt.savefig('results/adi.svg')