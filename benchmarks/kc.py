...
def bench(n, dtype):
    ...
    s = time.perf_counter()
    t = ta.volatility.KeltnerChannel(data, data, data, ov=False)
    t.keltner_channel_hband()
    t.keltner_channel_mband()
    t.keltner_channel_lband()
...
if __name__ == '__main__':
    ...
    plt.xlabel('n')
    plt.ylabel('speedup')
    plt.title('Keltner Channel (KC) Speedup')
    plt.legend(loc='upper right')
    plt.savefig('results/kc.svg')