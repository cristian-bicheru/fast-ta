...
double** _DC_DOUBLE(const double* high, const double* low, int len, int n) {
    ...
    _double_running_max(high, len, n, dc[0]);
    _double_running_min(low, len, n, dc[2]);
    _double_pairwise_mean(dc[0], dc[2], len, dc[1]);
    return dc;
}

float** _DC_FLOAT(const float* high, const float* low, int len, int n) {
    float** dc = float_malloc2d(3, len);
    _float_running_max(high, len, n, dc[0]);
    _float_running_min(low, len, n, dc[2]);
    _float_pairwise_mean(dc[0], dc[2], len, dc[1]);
    return dc;
}
...
double** _KC_DOUBLE(const double* high, const double* low, const double* close,
    ...
    for (int i = 0; i < num_channels; i++) {
        _double_add_arr(dc[num_channels+i], dc[0], len, dc[num_channels+i+1]);
    }

    for (int i = 0; i < num_channels; i++) {
        _double_sub_arr(dc[num_channels-i], dc[0], len, dc[num_channels-i-1]);
    }
    return dc;
}
float** _KC_FLOAT(const float* high, const float* low, const float* close,
    ...
    for (int i = 0; i < num_channels; i++) {
        _float_add_arr(dc[num_channels+i], dc[0], len, dc[num_channels+i+1]);
    }

    for (int i = 0; i < num_channels; i++) {
        _float_sub_arr(dc[num_channels-i], dc[0], len, dc[num_channels-i-1]);
    }
    return dc;
}